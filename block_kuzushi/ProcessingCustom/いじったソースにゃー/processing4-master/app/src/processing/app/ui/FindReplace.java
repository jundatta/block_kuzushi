...
//    layout.linkSize(SwingConstants.HORIZONTAL, findButton, replaceButton, replaceAllButton, replaceAndFindButton, previousButton);
  ...
  public void handleClose() {
	...
	// 単語単位で検索する
	private int wordUnitIndexOf(String text, String searchTerm,int index) {
		int nextIndex = 0;
		int start = index;
		while (nextIndex != -1) {
			// 検索する
			nextIndex = text.indexOf(searchTerm, start);
			// 見つからなかった場合は終わる
			if (nextIndex == -1) {
				return nextIndex;
			}
	...
	// 戻り方向に単語単位で検索する
	private int lastWordUnitIndexOf(String text, String searchTerm,int index) {
		int nextIndex = index;
		int start = index;
		while (nextIndex != -1) {
			// 検索する
			nextIndex = text.lastIndexOf(searchTerm, start);
			// 見つからなかった場合は終わる
			if (nextIndex == -1) {
				return nextIndex;
			}
...
//      editor.getSelectionStart() : editor.getSelectionStop();
      ...
      if (!backwards) {
        ...
        nextIndex = wordUnitIndexOf(text, searchTerm, selectionEnd);
        if (nextIndex == -1 && wrap && !allTabs) {
          // if wrapping, a second chance is ok, start from beginning
          nextIndex = wordUnitIndexOf(text, searchTerm, 0);
        ...
        if (selectionStart >= 0) {
          nextIndex = lastWordUnitIndexOf(text, searchTerm, selectionStart);
        } else {
          nextIndex = -1;
        }
        if (wrap &&  !allTabs && nextIndex == -1) {
          // if wrapping, a second chance is ok, start from the end
          nextIndex = lastWordUnitIndexOf(text, searchTerm, text.length());
