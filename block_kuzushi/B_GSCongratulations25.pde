...
//
// こちらがオリジナルです。
// 【作者】Joshua Rohwederさん
// 【作品名】Bounce the Ball
// https://openprocessing.org/sketch/1184774
//

class GameSceneCongratulations25 extends GameSceneCongratulationsBase {
  // bounce
  ...
  @Override void draw() {
    ...
    float zz = BALL_R/2.0+(BOUNCE_HEIGHT*(float)Math.abs(Math.sin(getCount()/BOUNCE_STRIDE))); // up-down
    ...
    if (!gameover) {
      ...
      pushMatrix();
      translate(xx, yy, zz);
      rotateX(-getCount()/20.0); // ball roll
      sphereDetail(8);
      fill(190, 56, 78);
      sphere(BALL_R);
      popMatrix();
