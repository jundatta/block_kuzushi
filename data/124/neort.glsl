...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    for(int i = 0; i < N; i++) {
        ...
        col = mix(col, INTENSITY, dirt(hash(vec2(ii)) + uv * max(0.0, t) * 0.1 + vec2(.2, -.2) * time, 3.5));
        ...
        col = mix(col, INTENSITY * vec3(1.), dirt(hash(vec2(ii)) + uv * max(0.0, t) * 0.1 + vec2(-.2, -.2) *  time, 3.5));
