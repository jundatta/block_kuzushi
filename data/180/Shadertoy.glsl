...
vec3 phongIllumination(vec3 k_a, vec3 k_d, vec3 k_s, float alpha, vec3 p, vec3 eye) {
    ...
    float dist = shortestDistanceToSurface(p, light1Dir, MIN_DIST + 0.003, MAX_DIST);
    if (dist < MAX_DIST)
        light1Intensity *= 0.0;
    ...
    dist = shortestDistanceToSurface(p, light2Dir, MIN_DIST + 0.003, MAX_DIST);
    if (dist < MAX_DIST)
        light2Intensity *= 0.0;
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    for (int i = 0; i < REFLECTIONS; i++) {
        ...
        color += phongIllumination(K_a, K_d, K_s, shininess, p, eye) * factor;
